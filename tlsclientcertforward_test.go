package tlsclientcertforward_test
...
type TestHttpResData struct {
	cfgSnHeaderName string
	certs           []string
	expectedSnValue string
}

func TestOneCert(t *testing.T) {
	data := TestHttpResData{
		cfgSnHeaderName: "Forwarded-Tls-Client-Cert-Dec-Sn",
	}

	t.Run("one minimalCheeseCrt", func(t *testing.T) {
		data.certs = []string{minimalCheeseCrt}
		data.expectedSnValue = "481535886039632329873080491016862977516759989652"
		testHttpRequest(t, data)
	})
	t.Run("one completeCheeseCrt", func(t *testing.T) {
		data.certs = []string{completeCheeseCrt}
		data.expectedSnValue = "1"
		testHttpRequest(t, data)
	})
	t.Run("one minimalCert", func(t *testing.T) {
		data.certs = []string{minimalCert}
		data.expectedSnValue = "12262702523050669712"
		testHttpRequest(t, data)
	})
}

func TestTwoCert(t *testing.T) {
	data := TestHttpResData{
		cfgSnHeaderName: "Forwarded-Tls-Client-Cert-Dec-Sn",
	}

	t.Run("one minimalCheeseCrt and completeCheeseCrt", func(t *testing.T) {
		data.certs = []string{minimalCheeseCrt, completeCheeseCrt}
		data.expectedSnValue = "481535886039632329873080491016862977516759989652"
		testHttpRequest(t, data)
	})
	t.Run("one minimalCheeseCrt and minimalCert", func(t *testing.T) {
		data.certs = []string{minimalCheeseCrt, minimalCert}
		data.expectedSnValue = "481535886039632329873080491016862977516759989652"
		testHttpRequest(t, data)
	})
	t.Run("one completeCheeseCrt and minimalCheeseCrt", func(t *testing.T) {
		data.certs = []string{completeCheeseCrt, minimalCheeseCrt}
		data.expectedSnValue = "1"
		testHttpRequest(t, data)
	})
	t.Run("one completeCheeseCrt and minimalCheeseCrt", func(t *testing.T) {
		data.certs = []string{completeCheeseCrt, minimalCert}
		data.expectedSnValue = "1"
		testHttpRequest(t, data)
	})
	t.Run("one minimalCert and completeCheeseCrt", func(t *testing.T) {
		data.certs = []string{minimalCert, completeCheeseCrt}
		data.expectedSnValue = "12262702523050669712"
		testHttpRequest(t, data)
	})
	t.Run("one minimalCert and minimalCheeseCrt", func(t *testing.T) {
		data.certs = []string{minimalCert, minimalCheeseCrt}
		data.expectedSnValue = "12262702523050669712"
		testHttpRequest(t, data)
	})
}
...
func testHttpRequest(t *testing.T, data TestHttpResData) {
	...
	cfg := tlsclientcertforward.CreateConfig()
	cfg.SnHeaderName = data.cfgSnHeaderName
	cfg.LogLevel = "warn"
	...
	handler, err := tlsclientcertforward.New(ctx, next, cfg, "tlsclientcertforward")
	if err != nil {
		t.Fatal(err)
	}
	...
	if data.certs != nil && len(data.certs) > 0 {
		req.TLS = buildTLSWith(data.certs)
	}
	...
	assertHeader(t, req, data.cfgSnHeaderName, data.expectedSnValue)
}
